...
function SIdebar({setinv, setSales, setpro}) {

  const sideref = useRef();

  const togglesidebar = () => {

    if (sideref.current.classList.contains("-left-full")) {
      sideref.current.classList.remove("-left-full");
      sideref.current.classList.add("left-0");
    } else if (sideref.current.classList.contains("left-0")) {
      sideref.current.classList.add("-left-full");
      sideref.current.classList.remove("left-0");
    }
  };
  ...
  return (
    <>
          ...
          <div className=" absolute left-5 sm:top-[5.5rem] top-[6rem] ">
            <button
      ...
      <div
        ref={sideref}
        className=" absolute bg-[#f7f7f8] h-screen -left-full w-[250px] py-6 px-4 font-[sans-serif] transition-all  "
      >
    
          <div className=" lg:left-48 z-10">
            <button
              onClick={togglesidebar}
              className=" top-2 h-6 w-6 cursor-pointer flex items-center justify-center rounded-full"
            >
